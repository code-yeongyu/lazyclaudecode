...
function ultragoalError(message, code) {
...
export async function recordFinalReviewBlockers(scope, args) {
    return withUltragoalMutationLock(scope, async () => {
        ...
        if (goal === undefined)
            ultragoalError(`Unknown ultragoal id: ${args.goalId}`, "ultragoal_goal_not_found");
        if (goal.status !== "in_progress")
            ultragoalError(`${goal.id} is ${goal.status}.`, "ultragoal_goal_not_in_progress");
        if (!isFinalRunCompletionCandidate(plan, goal))
            ultragoalError(`${goal.id} is not final.`, "ultragoal_not_final_story");
        ...
        if (args.goalSnapshotJson !== undefined) {
            ...
            if (!reconciliation.ok)
                ultragoalError(reconciliation.errors.join(" "), "ultragoal_goal_snapshot_mismatch");
        }
        const now = iso();
        ...
        const blockedEntry = { at: now, kind: "goal_review_blocked", goalId: goal.id, status: goal.status, evidence: args.evidence, ...(goalSnapshot === undefined ? {} : { goalSnapshot }) };
        ...
        const summaryEntry = { at: now, kind: "goal_review_blocked", goalId: goal.id, status: goal.status, evidence: args.evidence, ...(goalSnapshot === undefined ? {} : { goalSnapshot }), message: `Review blockers recorded; appended ${newGoal.id}.` };
        ...
        for (const entry of ledgerEntries)
            await appendLedger(scope, entry);
