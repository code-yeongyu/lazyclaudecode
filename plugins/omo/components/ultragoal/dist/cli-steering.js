...
function isKind(value) { return value !== undefined && ULTRAGOAL_STEERING_MUTATION_KINDS.some((kind) => kind === value); }
function isSource(value) { return value !== undefined && SOURCES.some((source) => source === value); }
function isModel(value) { return ULTRAGOAL_SUCCESS_CRITERION_USER_MODELS.some((model) => model === value); }
function fail(message, code, details) { throw new UltragoalError(message, code, { details }); }
function text(value, field) { if (value === undefined)
    return undefined; const trimmed = value.trim(); if (trimmed.length > 0)
    return trimmed; return fail(`Empty ${field}.`, "ULTRAGOAL_STEERING_FIELD_EMPTY", { field }); }
function required(argv, flag) { const value = text(readValue(argv, flag), flag); return value ?? fail(`Missing ${flag}.`, "ULTRAGOAL_STEERING_FIELD_REQUIRED", { flag }); }
function requiredGoal(argv) { const value = text(parseGoalArg(argv), "--goal-id"); return value ?? fail("Missing --goal-id.", "ULTRAGOAL_GOAL_ID_REQUIRED", { flag: "--goal-id" }); }
...
async function children(argv, flag, needed) {
    ...
    if (!Array.isArray(raw))
        return fail(`${flag} must be a JSON array.`, "ULTRAGOAL_STEERING_JSON_ARRAY_REQUIRED", { flag });
    const parsed = [];
    for (const item of raw) {
        const next = child(item);
        if (next === null)
            return fail(`${flag} entries require title/objective.`, "ULTRAGOAL_STEERING_CHILD_INVALID", { flag });
        parsed.push(next);
    }
    return parsed;
}
async function stringArray(argv, flag) {
    ...
    for (const item of raw) {
        if (typeof item !== "string")
            return fail(`${flag} entries must be strings.`, "ULTRAGOAL_STEERING_STRING_ARRAY_REQUIRED", { flag });
        values.push(text(item, flag) ?? "");
    }
    return values;
}
function model(value) { const trimmed = text(value, "--user-model"); if (trimmed === undefined)
    return undefined; return isModel(trimmed) ? trimmed : fail(`Invalid --user-model: ${trimmed}.`, "ULTRAGOAL_STEERING_USER_MODEL_INVALID", { value: trimmed, expected: ULTRAGOAL_SUCCESS_CRITERION_USER_MODELS }); }
function neverKind(kind) { return fail(`Unsupported steering kind: ${String(kind)}.`, "ULTRAGOAL_STEERING_KIND_UNSUPPORTED", { kind }); }
    ...
    switch (kind) {
        ...
        case "revise_pending_wording": {
            ...
            if (revisedTitle === undefined && revisedObjective === undefined)
                return fail("revise_pending_wording requires --title or --objective.", "ULTRAGOAL_STEERING_UPDATE_REQUIRED", { kind });
        ...
        case "revise_criterion": {
            ...
            if (scenario === undefined && expectedEvidence === undefined && userModel === undefined)
                return fail("revise_criterion requires scenario, expected-evidence, or user-model.", "ULTRAGOAL_STEERING_UPDATE_REQUIRED", { kind });
