...
function ultragoalFail(message, code, details) { throw new UltragoalError(message, code, { details }); }
...
function nonEmptyEvidence(evidence) { const trimmed = evidence.trim(); return trimmed || ultragoalFail("Evidence must be a non-empty string.", "ULTRAGOAL_EVIDENCE_REQUIRED", {}); }
export async function recordEvidence(scope, args) {
...
export async function markCriteriaPendingResetForGoal(scope, goalId) {
    return withUltragoalMutationLock(scope, async () => {
        const plan = await readUltragoalPlan(scope);
