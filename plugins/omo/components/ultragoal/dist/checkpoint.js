...
function ultragoalFail(message, code) { throw new UltragoalError(message, code); }
function normalizeObjective(value) { return value.replace(/\s+/g, " ").trim(); }
function nonEmptyEvidence(value) { const trimmed = value.trim(); return trimmed || ultragoalFail("Evidence must be a non-empty string.", "ultragoal_evidence_required"); }
function findGoal(plan, goalId) { const goal = plan.goals.find((candidate) => candidate.id === goalId); return goal ?? ultragoalFail(`Unknown ultragoal id: ${goalId}.`, "ultragoal_goal_not_found"); }
...
async function snapshotObjectiveMapsToUltragoalPlan(scope, snapshotObjective) {
    const actual = normalizeObjective(snapshotObjective).toLowerCase();
    if (textMentionsUltragoalPlanArtifact(actual))
        return true;
    if (actual.length < 24 || !existsSync(ultragoalBriefPath(scope)))
        return false;
    try {
...
async function canReconcileCompletedTaskScopedAggregateSnapshot(scope, plan, goal, snapshotObjective, evidence) {
    if (goalMode(plan) !== "aggregate")
        return false;
    ...
    if (!textMentionsUltragoalPlanArtifact(evidence) || !textMentionsGoalId(evidence, goal.id))
        return false;
...
async function readJsonInput(raw, repoRoot) {
    ...
    if (!existsSync(path))
        return ultragoalFail("Quality gate JSON is neither valid JSON nor a readable path.", "ultragoal_json_input_invalid");
    try {
        return JSON.parse(await readFile(path, "utf8"));
    }
    catch (error) {
        return ultragoalFail(`Quality gate path does not contain valid JSON${error instanceof Error ? `: ${error.message}` : "."}`, "ultragoal_json_input_invalid");
    }
}
function makeAggregateCompletion(now, evidence, goalSnapshot) {
...
function buildLedger(now, args, goal, qualityGate, goalSnapshot, aggregateCompletion) {
    ...
    if (goalSnapshot !== undefined)
        entry.goalSnapshot = goalSnapshot;
...
export async function checkpointUltragoal(scope, args) {
        ...
        if (args.status === "complete") {
            ...
            if (args.goalSnapshotJson !== undefined) {
                ...
                if (!reconciliation.ok) {
                    ...
                    if (!taskScoped)
                        throw new UltragoalError(`${formatGoalReconciliation(reconciliation)}${aggregate && snapshot?.status === "complete" && objective !== undefined ? buildTaskScopedAggregateReconciliationHint(goal, final) : ""}`, "ultragoal_goal_snapshot_mismatch");
