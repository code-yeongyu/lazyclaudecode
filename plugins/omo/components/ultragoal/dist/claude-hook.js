...
const GOAL_BUDGET_WARNING = "Do not set a budget on create_goal. Omit the budget field so the goal stays unlimited; ultrawork and ultragoal runs must always use unlimited goals.";
...
export async function applyUserPromptUltragoalSteering(payload) {
    try {
...
export function applyPreToolUseGoalBudgetGuard(payload) {
    ...
    if (payload.tool_name !== CREATE_GOAL_TOOL_NAME)
        return "";
    if (!hasGoalBudgetInput(payload.tool_input))
        return "";
    const output = {
        hookSpecificOutput: {
...
export async function runUltragoalHookCli(stdin, stdout) {
    try {
...
function hasGoalBudgetInput(value) {
