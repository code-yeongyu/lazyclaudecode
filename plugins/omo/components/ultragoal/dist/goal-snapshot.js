...
export function parseGoalSnapshot(value) {
...
export async function readGoalSnapshotInput(raw, cwd = process.cwd()) {
    if (!raw?.trim())
        return null;
    ...
    try {
        ...
        try {
            ...
            throw new GoalSnapshotError(`Goal snapshot path does not contain valid JSON: ${trimmed}${error instanceof Error ? ` (${error.message})` : ""}`);
        }
    }
}
export function reconcileGoalSnapshot(snapshot, options) {
...
export function formatGoalReconciliation(reconciliation) {
