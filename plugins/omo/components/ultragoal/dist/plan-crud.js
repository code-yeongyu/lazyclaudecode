...
function assertNonEmpty(value, label) { const trimmed = value?.trim(); if (!trimmed)
    throw new UltragoalError(`Missing ${label}.`, "ULTRAGOAL_ARGUMENT_MISSING"); return trimmed; }
...
export async function createUltragoalPlan(scope, args) {
    return withUltragoalMutationLock(scope, async () => {
        if (!args.force && existsSync(ultragoalGoalsPath(scope)))
            throw new UltragoalError(`Refusing to overwrite existing ${ULTRAGOAL_DIR}/sessions/${scope.sessionScope}/${ULTRAGOAL_GOALS}; pass --force to recreate it.`, "ULTRAGOAL_PLAN_EXISTS");
        const now = iso();
        ...
        const plan = { version: 2, platform: ULTRAGOAL_PLATFORM, sessionId: scope.sessionId, sessionScope: scope.sessionScope, createdAt: now, updatedAt: now, briefPath: `${sessionPrefix}/brief.md`, goalsPath: `${sessionPrefix}/${ULTRAGOAL_GOALS}`, ledgerPath: `${sessionPrefix}/${ULTRAGOAL_LEDGER}`, goalMode: args.goalMode ?? "aggregate", goals };
        if (plan.goalMode === "aggregate")
            plan.objective = ULTRAGOAL_AGGREGATE_OBJECTIVE;
...
export async function addUltragoalGoal(scope, args) {
    return withUltragoalMutationLock(scope, async () => {
...
export async function startNextUltragoal(scope, args = {}) {
    return withUltragoalMutationLock(scope, async () => {
        const plan = await readUltragoalPlan(scope);
        const now = iso();
        ...
        if (existing) {
            await appendLedger(scope, { at: now, kind: "goal_resumed", goalId: existing.id, status: existing.status, message: "Resuming active ultragoal" });
        ...
        if (!next && args.retryFailed) {
            ...
            if (next)
                await appendLedger(scope, { at: now, kind: "goal_retried", goalId: next.id, status: "pending", ...(next.failureReason ? { message: next.failureReason } : {}) });
        }
        if (!next)
            return { done: true, plan };
