...
import { legacyUltragoalGoalsPath, repoRelative, ultragoalGoalsPath, ultragoalIndexPath, ultragoalLedgerPath, ultragoalRootDir, ultragoalSessionDir, } from "./paths.js";
...
export async function withUltragoalMutationLock(scope, fn) {
...
function assertValidV2Plan(plan, scope, path) {
    if (plan.version !== 2 || !Array.isArray(plan.goals)) {
        throw new UltragoalError(`Invalid ultragoal plan at ${repoRelative(path, scope.repoRoot)}.`, "ULTRAGOAL_PLAN_INVALID");
    }
}
/**
 ...
 * place (never deleted).
 */
export async function readUltragoalPlan(scope) {
    ...
    try {
        ...
        if (migrated !== null)
            return migrated;
        throw new UltragoalError(`No ultragoal plan found at ${repoRelative(path, scope.repoRoot)}. Run \`omo ultragoal create-goals ...\` first.`, "ULTRAGOAL_PLAN_MISSING", { cause: error });
    }
    const parsed = JSON.parse(raw);
    assertValidV2Plan(parsed, scope, path);
    return await maybeMigrateAggregateObjective(scope, parsed);
}
async function maybeMigrateAggregateObjective(scope, plan) {
 ...
 */
async function tryMigrateLegacyV1(scope) {
    ...
    if (legacy.version !== 1 || !Array.isArray(legacy.goals))
        return null;
    const now = iso();
...
function migrateV1ToV2(legacy, scope) {
    ...
    if (legacy.codexGoalMode !== undefined)
        plan.goalMode = legacy.codexGoalMode;
    if (legacy.codexObjective !== undefined)
        plan.objective = legacy.codexObjective;
    if (legacy.codexObjectiveAliases !== undefined)
        plan.objectiveAliases = legacy.codexObjectiveAliases;
    if (legacy.activeGoalId !== undefined)
        plan.activeGoalId = legacy.activeGoalId;
...
export async function writePlan(scope, plan) {
...
export async function readSteeringLedgerEntries(scope) {
    let raw;
    try {
...
// --- index.json registry ---
export async function readUltragoalIndex(repoRoot) {
    try {
...
async function writeUltragoalIndex(repoRoot, index) {
...
async function upsertIndexEntry(scope, plan) {
