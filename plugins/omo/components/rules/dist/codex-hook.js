...
export async function runSessionStartHook(input, options = {}) {
    ...
    return runStaticInjection(input.cwd, transcriptPath, "SessionStart", cachePath, options, postCompactPending ? "static" : undefined, { latestCompactedReplacementOnly: postCompactPending });
}
...
export async function runUserPromptSubmitHook(input, options = {}) {
    ...
    return runStaticInjection(input.cwd, input.transcript_path, "UserPromptSubmit", cachePath, options, postCompactPending ? "static" : undefined, { latestCompactedReplacementOnly: postCompactPending });
}
...
function runStaticInjection(cwd, transcriptPath, eventName, cachePath, options, completedPostCompactChannel, transcriptSearchOptions = {}) {
    const config = configFromEnvironment(options.env);
